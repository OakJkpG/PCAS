...
# ─── Local Auth Check ───
# If token doesn't exist, we will trigger the browser login during startup.
if not os.path.exists("token.json") and os.path.exists("credentials.json"):
    print("🔐 First time login detected. Will prompt for Google Calendar Auth shortly.")
...
if __name__ == '__main__':
    # 0. Pre-flight check for Google Calendar Auth (Opens browser if needed)
    print("=" * 50)
    print("🔐 Checking Local Calendar Auth...")
    try:
        get_calendar_service()
        print("✅ Calendar Auth OK.")
    except Exception as e:
        print(f"⚠️ Calendar Auth might need browser login: {e}")
    print("=" * 50)
